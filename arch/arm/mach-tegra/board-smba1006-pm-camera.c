/*
 * arch/arm/mach-tegra/board-smba1006-pm-camera.c
 *
...
static int smba_s5k4cdgx_power_on(void)
{
...
int smba_s5k4cdgx_set_power(int enable)
{
  if(enable)
    return smba_s5k4cdgx_power_on();
  else
    return smba_s5k4cdgx_power_off();
}
EXPORT_SYMBOL(smba_s5k4cdgx_set_power);

static int smba_s5k4cdgx_reset(void)
{
